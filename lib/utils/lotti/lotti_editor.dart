...
class LottieEditor {
  ...
  Map<String, dynamic> _flutterColorToLottie(Color flutterColor) {
    ...
    final alpha = flutterColor.opacity;
  ...
  static Future<Uint8List> loadLottieData(String path) async {
    return await rootBundle.load(path).then((value) => value.buffer.asUint8List());
  }

  static Future<Uint8List> get onBoardingOne async {
    final editor = LottieEditor();
    await editor.openAndLoad('assets/lottie/onbo_a.json');
    editor.changeWholeLottieFileColor(Get.theme.colorScheme.tertiary);
    return editor.convertToUint8List();
  }
  
  static Future<Uint8List> get onBoardingTwo async {
    final editor = LottieEditor();
    await editor.openAndLoad('assets/lottie/onbo_b.json');
    editor.changeWholeLottieFileColor(Get.theme.colorScheme.tertiary);
    return editor.convertToUint8List();
  }
  
  static Future<Uint8List> get onBoardingThree async {
    final editor = LottieEditor();
    await editor.openAndLoad('assets/lottie/onbo_c.json');
    editor.changeWholeLottieFileColor(Get.theme.colorScheme.tertiary);
    return editor.convertToUint8List();
  }

   static Future<Uint8List> get notLoggedIn async {
    final editor = LottieEditor();
    await editor.openAndLoad('assets/lottie/no_login.json');
    editor.changeColorsOfShapeNames (['Grupo 1', 'Grupo 2','Grupo 3','Login 2','Login 3','Login 4','Login 5','Login 6','Login 7','Login 8'], Get.theme.colorScheme.tertiary.withOpacity(0.5));
    return editor.convertToUint8List();
  }

  static Future<Uint8List> get noLoggedIn async {
    final editor = LottieEditor();
    await editor.openAndLoad('assets/lottie/no_logged_in.json');
    editor.changeWholeLottieFileColor(Get.theme.colorScheme.tertiary);
    return editor.convertToUint8List();
  }
