...
class PropertyModel {
  ...
  factory PropertyModel.fromMap(Map<String, dynamic> rawjson) {
    try {
      final dynamic params = rawjson['parameters'];
      if (params is List) {
        // Optionally precache parameter images here if needed
        // final images = params.map((e) => e['image']).toList();
      }
    } catch (e) {
    ...
    return PropertyModel(
        ...
        gallery: (() {
          final dynamic rawGallery = rawjson["gallery"];
          if (rawGallery is List) {
            return List<Gallery>.from(
              rawGallery.map(
                (x) => Gallery.fromMap(x is String ? json.decode(x) : x),
              ),
            );
          }
          return <Gallery>[];
