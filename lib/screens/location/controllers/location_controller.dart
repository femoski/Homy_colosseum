...
class LocationController extends GetxController implements GetxService {
  ...
  Future<AddressModel> getCurrentLocation(bool fromAddress, {GoogleMapController? mapController, LatLng? defaultLatLng, bool notify = true}) async {
    ...
    Position myPosition = await getPosition(defaultLatLng, LatLng(
      double.parse('0'),
      double.parse('0'),
    ));
    fromAddress ? _position = myPosition : _pickPosition = myPosition;

    handleMapAnimation(mapController, myPosition);
    Get.log('myPosition: ${myPosition.latitude} ${myPosition.longitude}');
  ...
  Future<String> getAddressFromGeocode(LatLng latLng) async {
    return await locationRepository.getAddressFromGeocode(latLng);
    return 'test';
  ...
  void updatePosition(CameraPosition? position, bool fromAddress) async {
    if(_updateAddAddressData) {
      ...
      _buttonDisabled = !true;
      if (_changeAddress) {
          String addressFromGeocode = await locationRepository.getAddressFromGeocode(LatLng(position.target.latitude, position.target.longitude));
          fromAddress ? _address = addressFromGeocode : _pickAddress = addressFromGeocode;
      } else {
        _changeAddress = true;
      }
  ...
  Future<PlaceModel> setLocation(String? placeID, String? address, GoogleMapController? mapController) async {
    _loading = true;
    update();

    LatLng latLng = await getLatLng(placeID);
    ...
    _changeAddress = false;
        ...
        if(mapController != null) {
      mapController.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: latLng, zoom: 20)));
    }
    _loading = false;
    update();
    return PlaceModel(
  ...
  //   Response? response = await locationRepository.getPlaceDetailsFromPlaceId(id);
  ...
  Future<LatLng> getLatLng(String? placeId) async {
    final placeDetails = await locationRepository.getPlaceDetailsFromPlaceId(placeId);
    LatLng latLng = const LatLng(0, 0);

    placeDetails['placeId'] = placeId;
    latLng = LatLng(placeDetails['lat'] as double, placeDetails['lng'] as double);
    return latLng;
  }
  ...
  Future<List<PredictionModel>> searchLocation(BuildContext context, String text) async {
    if(text.isNotEmpty) {
      _predictionList = await locationRepository.searchLocation(text);
      Get.log('predictionList: ${_predictionList}');
    }
